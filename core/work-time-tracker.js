...
class WorkTimeTracker {
    constructor(config = {}) {
        ...
        // Use provided credentials or empty defaults
    ...
    parseWorkTimeFromHTML(htmlContent) {
        ...
        if (typeof document !== 'undefined') {
            ...
            // Node.js environment - force regex parsing for now due to jsdom issues
        ...
        for (const row of rows) {
            ...
            if (cells.length > 0) {
                ...
                if (dateCell === todayStr) {
                    // Look for specific column classes instead of relying on position
                    const presenceCell = row.querySelector('td.column6');
                    const detailCell = row.querySelector('td.column5');
                    
                    const presenceTime = presenceCell?.textContent?.trim() || '0';
                    const detailText = detailCell?.textContent?.trim() || '';
                    
                    const timestamps = this.parseTimestamps(detailText);
    ...
    calculateRemainingTime(data) {
        ...
        if (lastExit && data.entries.length === data.exits.length) {
            // Person has left - calculate based on exit time
            const lastExitMinutes = this.parseTimeToMinutes(lastExit);
            timeFromEntry = lastExitMinutes - entryMinutes;
        } else {
            // Person is still working - calculate based on current time
            const currentTime = new Date();
            const currentTimeMinutes = currentTime.getHours() * 60 + currentTime.getMinutes();
            timeFromEntry = currentTimeMinutes - entryMinutes;
        }
        ...
        if (remainingPresence <= 0 && remainingFromEntry <= 0) {
            ...
            // Check if requirements are met even though person has left
            if (remainingPresence <= 0 && remainingFromEntry <= 0) {
                status = 'Can leave now! ✅';
                workStatus = 'can_leave';
            } else {
